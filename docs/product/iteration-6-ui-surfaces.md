...
- **U3 — market map and rival profiles.** "Market size" is the topic's demand multiplier (the engine has no other per-topic size); the storm sprite never shows at the shipped balance; the rival card's tap target is its head row because the card carries the Acquire button; the buyout actions on the profile are only reachable while that rival's offer is pending; no snapshot of the whole Market section because `ImageRenderer` cannot draw a UIKit segmented picker. `Rival.strengthHistory` is weekly, capped at 52, encoded only when non-empty. PixelKit 297 → 318.
- **U6 — storefront.** Type picks the device chrome and topic the motif (6 × 12 distinct strips); "What's new" is thin because `ReleaseInfo` records only a patch count and the last patch day; the price button opens a sheet built on the live-ops action rather than routing away; `-autoRoute storefront` does not answer story questions. **Found, not fixed:** dismissing a presented sheet from outside the view tree (deferring a question, clearing `launchDayProductID`) can trap in an environment getter when sheet content reads a non-optional `@Environment(AppRouter.self)` while its host is torn down (`LaunchDaySheet.swift:15` is the named site); the reads should become optional. Also noted twice: `ProductBoxArt` keys on `"mobile"`/`"web"` while the type ids are `mobile_app`/`web_app`, so most products draw the monitor silhouette.

Final tree: engine 888, content 52, save 21, PixelKit 318, app 233.
